...
class ItemAPIView(ListAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        # 갖고 있는 아이템 목록
        user_owned_item_ids = UserItem.objects.filter(user=request.user).values_list("item__id", flat=True)
        ...
        for item_condition in ItemCondition.objects.all():
            if not item_condition.pre_item_condition:
                ...
                if self.check_condition(pre_item_condition, user_owned_items) and self.check_status(
                    item_condition, user_owned_items
                ):
                    buyable_item_ids.append(item_condition.item_id)
